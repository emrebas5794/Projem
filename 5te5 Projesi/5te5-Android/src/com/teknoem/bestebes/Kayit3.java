...
public class Kayit3 extends Activity  implements OnClickListener  {
	...
	String deger,url,uyeid,donenad,donensoyad,donenkullaniciadi,donensifre,donenmail;
	 private String secilenDeger="";
	    ...
	    ImageView mChart;
	     String android_id,takimresmi,takimadi,takimid,url2,version;
...
{
	 sifrele=donenmail.substring(1, donenmail.length()-1)+donenad.substring(1, donenad.length()-1)+donensoyad.substring(1, donensoyad.length()-1)+donenkullaniciadi.substring(1, donenkullaniciadi.length()-1)+android_id.substring(1, android_id.length()-1);
	
	 url = "http://teknoem.com/bestebes/uyekaydet.aspx?uyekaydet=true&eposta="+donenmail+"&takimi="+secilenDeger+"&ad="+donenad+"&soyad="+donensoyad+"&kullaniciadi="+donenkullaniciadi+"&sifre="+donensifre+"&androidid="+android_id+"&versiyon="+version+"&hasie="+sifrele+"";
	   				...
	   				uyeid=c.getString(TAG_uyeid);
	   				 if(deger.equals("true"))
	   				 {
	   			    	...
	   			    	Notification bildirim =	new Notification.Builder(getApplicationContext()).
	   			    							...
	   			    							setContentText("Kayýt Olduðunuz Ýçin Teþekür Ederiz.").
	 ...
	 private class JSONParse2 extends AsyncTask<String, String, JSONObject> {
        ...
        protected void onPreExecute() {
   		 ...
   		 if(nf != null && nf.isConnected()==true )
 		    {  
   			 pDialog = new ProgressDialog(Kayit3.this);
           pDialog.setMessage("Kayýt Yapýlýyor..");
           pDialog.setIndeterminate(false);
           pDialog.setCancelable(false);
           pDialog.show();
